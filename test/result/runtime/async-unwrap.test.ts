...
describe('with Result<Promise<X>>', testCase(
  <X> (x: X) => ok(Promise.resolve(x)),
  <X> (x: X) => err(Promise.resolve(x))
))

describe('with Promise<Result<X>>', testCase(
  async <X> (x: X) => ok(x),
  async <X> (x: X) => err(x)
))

describe('with Promise<Result<Promise<X>>>', testCase(
